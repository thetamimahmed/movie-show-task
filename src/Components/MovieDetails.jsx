...
const MovieDetails = () => {
    ...
    const { id, name, type, language, genres, status, premiered, ended, schedule, rating, network, summary, image } = movie
    return (
        <div  data-aos="zoom-in-down" data-aos-duration="1000" className="mt-5 mb-10 ">
            <div className="mx-4 lg:mx-0">
                <img className="w-1/2 lg:w-1/3 mx-auto" src={image.original} alt="" />
                <h1 className="text-center text-4xl mt-3 font-bold text-[#eb2f06]">{name}</h1>
                <div className="text-xl font-bold md:flex justify-evenly mt-4">
                ...
                <div className="text-center">
                    ...
                    <Link to={`/book/${id}`}><button className="btn btn-primary bg-[#eb2f06] hover:bg-[#e55039] border-0 text-white">Book Movie</button></Link>
                </div>
            </div>
        </div>
    );
